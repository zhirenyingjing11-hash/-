 ...
 *     → URLを丸ごと貼ってもOK（自動でIDを抜き出します）
 *   ・無ければ getActive（スプレッドシートに紐づくバインド型スクリプト向け）
 */
function getSS_() {
  let id = String(getConfig_().SPREADSHEET_ID || '').trim();
  if (id) {
    // スプレッドシートのURL（.../d/★ID★/edit...）が貼られていてもIDを抽出
  ...
  if (!active) {
    throw new Error('スプレッドシートが特定できません。スクリプトプロパティ SPREADSHEET_ID にシートのURL（またはID）を設定してください。');
  }
  return active;
}
